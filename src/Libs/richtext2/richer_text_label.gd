...
static var STACK_STATE := preload("res://src/Libs/richtext2/stack_state.gd").new()
...
const DIR_TEXT_EFFECTS := "res://src/Libs/richtext2/text_effects/effects"
const DIR_TEXT_TRANSITIONS := "res://src/Libs/richtext2/text_effects/anims"
...
func _meta_clicked():
	...
	if _meta_hovered.begins_with("https://") and meta_auto_https:
		...
		# var gott : Variant = get_expression(_meta_hovered)
...
func _update_theme_outline():
	...
	match outline_mode:
		...
		OutlineMode.DARKEN:
			add_theme_color_override(&"font_outline_color", hue_shift(default_color.darkened(outline_adjust), outline_hue_adjust))
		OutlineMode.LIGHTEN:
			add_theme_color_override(&"font_outline_color", hue_shift(default_color.lightened(outline_adjust), outline_hue_adjust))
...
func _replace(string: String, regex: RegEx, callshit: Callable) -> String:
	var offset := 0
	var output := ""
	while true:
		...
		output += str(callshit.call(m.strings))
		offset = m.get_end(0)
...
func replace_context(string: String) -> String:
	...
	string = _replace(string, REGEX_REPLACE_CONTEXT_2, func(strings):
		var expshit: String = strings[0]
		return _get_expression_rich(expshit, unwrap(expshit, "{}")))
	
	return string

func _get_expression_rich(expshit: String, exp_clean: String) -> String:
	var value = get_expression(exp_clean)
	if _expression_error != OK:
		return "[red]%s[]" % expshit
	else:
...
func _add_text(t: String):
	if STACK_STATE.pipes:
		var expshit := "|".join([var_to_str(t)] + STACK_STATE.pipes)
		var got = get_expression(expshit)
		t = str(got)
		append_text(t)
		return
...
func _push_paragraph(alignshit :int):
	_stack_push(T_PARAGRAPH, STACK_STATE.align)
	STACK_STATE.alignshit = alignshit
	push_paragraph(alignshit)
	...
	for ext in IMAGE_EXTENSIONS:
		...
		if FileAccess.file_exists(full_path):
			...
			if image_resize_to_font_size:
				...
				wide = (aspect as int) * font_size
				high = font_size
			if image_scale != 1.0:
				wide *= image_scale as int
				high *= image_scale as int
...
func _push_font(fontshit: String):
	_stack_push(T_FONT, STACK_STATE.font)
	STACK_STATE.font = fontshit
...
func _push_color(clr: Color):
	...
	if outline_size > 0:
		if outline_mode in [OutlineMode.DARKEN, OutlineMode.LIGHTEN, OutlineMode.CUSTOM_DARKEN, OutlineMode.CUSTOM_LIGHTEN]:
			# Outline color.
			var outline_colorshit := _get_outline_color(clr)
			push_outline_color(outline_colorshit)
...
func _prop_group(list: Array[Dictionary], nameshit: String, hint_string: String):
	list.append({ name=nameshit, type=TYPE_NIL, usage=PROPERTY_USAGE_GROUP, hint_string=hint_string })

func _prop_subgroup(list: Array[Dictionary], nameshit: String, hint_string: String):
	list.append({ name=nameshit, type=TYPE_NIL, usage=PROPERTY_USAGE_SUBGROUP, hint_string=hint_string })

func _prop_enum(list: Array[Dictionary], nameshit: StringName, en: Variant):
	_prop(list, nameshit, TYPE_INT, PROPERTY_HINT_ENUM, ",".join(en.keys().map(func(s): return s.capitalize())))

func _prop_range(list: Array[Dictionary], nameshit: StringName, type: int = TYPE_FLOAT, minn = 0.0, maxx = 1.0):
	list.append({ name=nameshit, type=type, usage=PROPERTY_USAGE_DEFAULT, hint=PROPERTY_HINT_RANGE, hint_string="%s,%s" % [minn, maxx] })

func _prop_node(list: Array[Dictionary], nameshit: StringName, hint_string: String = ""):
	list.append({ name=nameshit, type=TYPE_OBJECT, usage=PROPERTY_USAGE_DEFAULT, hint=PROPERTY_HINT_NODE_TYPE, hint_string=hint_string })

func _prop(list: Array[Dictionary], nameshit: StringName, type: int, hint: PropertyHint = PROPERTY_HINT_NONE, hint_string: String = ""):
	list.append({ name=nameshit, type=type, usage=PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_SCRIPT_VARIABLE, hint=hint, hint_string=hint_string })
